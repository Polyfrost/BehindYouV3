...
object Updater {
     ...
     */
    fun update() {
        CoroutineScope(Dispatchers.IO + CoroutineName("${BehindYou.NAME}-UpdateChecker")).launch {
            val latestRelease =
                APIUtil.getJSONResponse("https://api.github.com/repos/Wyvest/${BehindYou.ID}/releases/latest")
            latestTag = latestRelease.get("tag_name").asString

            val currentVersion = DefaultArtifactVersion(BehindYou.VERSION.substringBefore("-"))
            val latestVersion = DefaultArtifactVersion(latestTag.substringAfter("v").substringBefore("-"))

            if ((BehindYou.VERSION.contains("BETA") && currentVersion >= latestVersion)) {
                return@launch
